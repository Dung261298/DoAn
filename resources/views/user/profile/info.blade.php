		...
		<div class="row">
			<div class="col-lg-12 col-md-12 col-sm-12">	
				<div class="breadcrumb-row">
					<h3 class="breadcrumb"><a href="/" class="home">Home</a><span>/</span>{{__('profileUser.profile')}}</h3>
				</div>
			</div>
		</div>
	</div>
	...
	<div class="row">
		...
		<div class="col-xs-12 col-sm-3 info_user">
			<div class="info_user_inner">
				<h2 class="text-center line_green_center">{{__('profileUser.profile')}}</h2>
				@if (session('message'))
			...
			<div class="text-center">
				<a href="{{route('profile.edit',Auth::guard('client')->user()->username)}}" class="btn btn-success">{{__('profileUser.edit')}}</a>
				<a href="{{ url('/changepassword') }}" class="btn btn-primary">{{__('profileUser.editpass')}}</a>
			</div>
		</div>
		<div class="col-xs-12 col-sm-8" style="background: #f5f5f5;"> 
			<ul class="nav nav-tabs orderstatus">
				...
				<li class="delivery">
					<a class="h4" href="{{url('/profile')}}?status=delivery" style="color: #444;margin: 0;">{{__('profileUser.delivery')}}(<span class="text-danger" style="font-size: 15px; font-weight: bold;">{{$quantity[2]}}</span>)</a>
				</li> 
				<li class="delivered">
					<a class="h4" href="{{url('/profile')}}?status=delivered" style="color: #444;margin: 0;">{{__('profileUser.delivered')}}(<span class="text-danger" style="font-size: 15px; font-weight: bold;">{{$quantity[3]}}</span>)</a>
				</li> 
				<li class="cancel">
					<a class="h4" href="{{url('/profile')}}?status=cancel" style="color: #444;margin: 0;">{{__('profileUser.cancel')}}(<span class="text-danger" style="font-size: 15px; font-weight: bold;">{{$quantity[4]}}</span>)</a>
				</li> 
			</ul>
			<br>
			...
			<div class="card col-md-12 bg-light" style="margin-bottom: 15px; background: white;border-radius: 4px;">
				<div class="row no-gutters" style="margin: 10px 0px;">
					...
					<div class="col-xs-3 col-md-3">
						...
						<p>{{__('profileUser.totalamount')}}: <span style="color: green; font-size: 20px;">{{$order->total_amount}}đ</span></p> 
						<a href="{{ url('order/'.$order->order_code) }}" class="btn btn-success col-xs-12 col-md-5" style="margin-right: 10px;">{{__('profileUser.detail')}}</a>
